...
describe("MarketCapSortedTokenCategories.sol", () => {
  ...
  let erc20Factory, categoryOracle;
  ...
  before(async () => {
    ...
    await bre.run('deploy_category_oracle');
    ({
      from,
      uniswapFactory,
      uniswapRouter,
  ...
  describe('Initialize Oracle', async () => {
    it('Should create a wrapped tokens category', async () => {
      ...
      const receipt = await categoryOracle.createCategory(metadataHash);
    ...
    it('Should add tokens to the wrapped tokens category', async () => {
      await categoryOracle.addTokens(1, wrappedTokens.map(t => t.address)).then(r => r.wait());
    });

    it('Should query the category tokens', async () => {
      const tokens = await categoryOracle.getCategoryTokens(1);
    ...
    it('Should fail to return a price if the latest price is not old enough', async () => {
      const [token] = await categoryOracle.getCategoryTokens(1);
      expect(
    ...
    it('Should return the correct market caps', async () => {
      ...
      for (let i = 0; i < wrappedTokens.length; i++) {
        ...
        const realMarketCap = await categoryOracle.computeAverageMarketCap(address)
          .then(toBN);
        const pct = realMarketCap.div(expectedMarketCap);
        expect(pct.eqn(1)).to.be.true;
        caps.push(realMarketCap.toString('hex'));
      }
      const categoryCaps = await categoryOracle.getCategoryMarketCaps(1);
  ...
  describe('Sort Tokens', async () => {
    ...
    async function getCategoryData(id) {
      const tokens = await categoryOracle.getCategoryTokens(id);
      const marketCaps = await categoryOracle.getCategoryMarketCaps(id);
      const arr = [];
    ...
    it('Should sort the tokens and update the category', async () => {
      ...
      const receipt = await categoryOracle.orderCategoryTokensByMarketCap(
