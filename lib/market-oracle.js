...
module.exports = class MarketOracle {
  ...
  static async deploy(web3, uniswapFactoryAddress, stablecoinAddress, from) {
  ...
  async createCategory(metadataHash) {
    const receipt = await this.oracle.methods.createCategory(metadataHash).send({ from: this.from });
  ...
  async sortCategoryTokens(categoryID) {
    const { lastUpdated, tokens } = await this.getCategory(categoryID);
    ...
    return this.marketOracle.methods.orderCategoryTokensByMarketCap(
      categoryID,
